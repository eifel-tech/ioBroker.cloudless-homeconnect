	...
	entries
		...
		.forEach((el) => {
			...
			keys.filter((key) => key === "execution").forEach(() => {
				if (el.option) {
					//Manchmal kann bei einer globalen Option access="none" sein. Dann wird der Wert aus der Option des Programms genommen,
					// der dann oft access="read" oder access="readwrite" ist.
					Object.values(el.option).forEach((option) => {
						const opt = parsedFeatures[parseInt(option.$.refUID, 16)];
						if (opt.access === "none") {
							opt.access = option.$.access;
						}
					});
